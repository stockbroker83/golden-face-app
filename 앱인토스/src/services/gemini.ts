...
⭐ 궁합 분석 작성 규칙 (품질 10배 향상):

**8가지 궁합 항목 (각 3-4문장)**:
1. **연애 궁합**: 감정 교류 방식 + 데이트 스타일 + 갈등 해결 패턴
2. **우정 궁합**: 공통 관심사 + 신뢰 형성 과정 + 장기 우정 가능성
3. **업무 궁합**: 협업 스타일 + 역할 분담 + 시너지 포인트
4. **대화 궁합**: 소통 리듬 + 경청 능력 + 오해 발생 가능성
5. **가치관 궁합**: 인생 목표 일치도 + 금전관 + 가족관
6. **취미 궁합**: 여가 활동 조화 + 개인 시간 존중도
7. **성장 궁합**: 서로에게 주는 긍정적 영향 + 발전 가능성
8. **갈등해결 궁합**: 다툼 패턴 + 화해 방식 + 스트레스 대처

**상세 조언**:
- **complementary**: 서로 보완하는 점 4-5문장 (구체적 예시 3가지 이상)
- **warning**: 주의할 점 4-5문장 (실제 상황 + 대처법)
- **advice**: 관계 개선 조언 5-6문장 (실천 가능한 행동 5가지)

**추가 필드**:
- conflict_resolution: 갈등 해결 3단계 가이드
- growth_potential: 함께 성장할 수 있는 영역 3가지
- date_recommendations: 추천 데이트 장소/활동 3가지
- communication_tips: 대화 팁 3가지
- long_term_forecast: 1년 후, 3년 후, 5년 후 관계 전망
